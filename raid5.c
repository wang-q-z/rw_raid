...
#define ths_sector 128    

int is_large(struct bio * bi)
{
		int size = bi->bi_iter.bi_size;
        if(size > ths_sector)
                return 0;
        else
                return 0;
...
static bool raid5_make_request(struct mddev *mddev, struct bio * bi)
{
	...
	struct rwconf *rw_conf;
	//在这里加把锁，进行复制写的初始化,只进行一次初始化
	spin_lock(&conf->init_lock);
	if(conf->is_init == 0)
	{
		rw_conf = Init_RWConf(conf);
		conf->is_init = 1;
	};//未进行初始化
	spin_unlock(&conf->init_lock);
	...
	{
//		submit_bio_noacct(bi);
		//pr_debug("do replicated writing--------------------------------------\n");
		//bio_put(bi);
		printk("----------------start to rw -------------------\n");
		Do_RW(bi,rw_conf);
		printk("----------------finish rw -------------------\n");
